...
**US7 / FR-016 (structure presets) ANULADAS** por decisi�n del usuario: todo documento nuevo nace **completamente vac�o** (`content_html = ''`). Los skeletons Letter/Official Letter fueron removidos del producto y del c�digo (`presets.ts` eliminado). El campo `format_type` persiste en el modelo por compatibilidad, fijo en `'blank'` para documentos nuevos.
