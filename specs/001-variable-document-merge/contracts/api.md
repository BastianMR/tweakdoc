...
- Import response columns incluyen `type` detectado autom�ticamente.
- PUT persiste `type`; sanitizaci�n client-side: number rechaza no-num�rico (celda marcada inv�lida), date exige ISO.
- Inyecci�n por API ignora tipos (valores string) � validaci�n de forma ocurre al editar en UI.
